  ...
  const fetchWorkout = useCallback(async () => {
    try {
      ...
      setWorkout({
        ...
        duration_minutes: data.workout.duration_minutes || 45,
      })
      
      setExercises(data.workout.exercises.map((ex: Exercise) => ({
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-10">
      ...
      <div className="p-5 flex items-center gap-4">
        <Link href={`/workouts/${workoutId}`} className="text-zinc-500 hover:text-white">
          ← Cancel
        </Link>
        <h1 className="text-xl font-black">Edit Workout</h1>
      </div>
